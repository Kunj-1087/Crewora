...
export function CustomerLoginForm() {
  ...
  const verify = async (value: string) => {
    ...
    try {
      await loginCustomer(getValues('phone'), value);
      router.push('/customer/dashboard');
    } catch {
      setWrongOtp(true); // triggers shake on the OTP input
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
      ...
      {/* Developer-mode OTP helper (only shown when the API returns an OTP). */}
      <ConfirmationModal
